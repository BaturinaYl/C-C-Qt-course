...
//-----------------------------------------------------------------
    class Buy : public Item {
     protected:
           ...
           int m_qnt;
      public:
           Buy (int q = 0, string nameI="", int priceI = 0, int weightI = 1):
...
//-----------------------------------------------------------------
  ...
  public:
    Check (int q=0, string nameI="", int priceI=0, int weightI = 1) : Buy(q, nameI, priceI, weightI)
         {
           }
    int Sum (int *s){
...
// создаем и инициализируем список товаров
    ...
    for( int k = 0; k < n; k++)
      ...
      Check ch1(move(Check(buyIt[1].getqnt(), buyIt[1].getname(), buyIt[1].getprice(), buyIt[1].getweight())));
      ch1.print_list_buy();     
     delete[] buyIt;
return 0;
}
